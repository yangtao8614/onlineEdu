...
<div class="page-container">
    <div class="text-c"> 日期范围：
        ...
        <input type="text" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d' })" id="datemax"
               class="input-text Wdate" style="width:120px;">
        <input type="text" class="input-text" style="width:250px" placeholder="输入课程名称" id="title" name="">
        <button type="submit" class="btn btn-success radius" id="searchTitle" name=""><i
                    class="Hui-iconfont">&#xe665;</i> 搜课程
        </button>
    </div>
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a href="javascript:;" onclick="datadel()"
                                                               class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a
                    href="javascript:;" onclick="lesson_add()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加课程</a></span>
        <span class="r">共有数据：<strong>{{count($course)}}</strong> 条</span></div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort" style="text-align: center">
            ...
            @foreach($course as $b)
                <tr style="text-align: center">
                    ...
                    <td style="text-align: center">{{$b->course_name}}</td>
                    <td style="text-align: center">{{$b->profession->profession_name}}</td>
                    <td style="text-align: center">{{$b->course_desc}}</td>
...
//                    mytable.api().ajax.reload();
    ...
    function lesson_add() {
        ...
        layer_show('添加课程', '{{url("admin/course/add")}}');
    }
    ...
    function lesson_edit(id) {
        layer_show('修改课程', '{{url("admin/lesson/update")}}/' + id);
    }
