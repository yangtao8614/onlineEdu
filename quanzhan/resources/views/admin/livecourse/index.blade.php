...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="live_add()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加直播课程</a></span> <span class="r">共有数据：<strong>54</strong> 条</span> </div>
	<table class="table table-border table-bordered table-bg">
		...
		@foreach($livecourse as $v)
			<tr class="text-c">
				...
				<td><input onclick="getPush({{$v->id}},{{$v->stream_id}})" class="btn btn-success-outline radius" type="button" value="推流地址"></td>
...
*/
/*生成推流地址的函数*/
function getPush(id,stream_id){
	//实现一个弹窗效果
	layer_show('生成推流地址','/admin/livecourse/getPush/'+id+'/'+stream_id,800,200);
}

/*直播课程-增加*/
function live_add(){
	layer_show('添加直播课程','{{url("admin/livecourse/add")}}');
}
/*管理员-删除*/
